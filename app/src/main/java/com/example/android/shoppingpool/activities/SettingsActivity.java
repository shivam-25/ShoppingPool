...
import com.example.android.shoppingpool.models.Contact;
import com.example.android.shoppingpool.utilities.AppCompatPreferenceActivity;
...
/*    @Override
    ...
    private static void updateFirebaseDB(AppUser user) {
        // update Firebase DB Online
        DatabaseReference databaseRef;
        databaseRef = FirebaseDatabase.getInstance().getReference();
        databaseRef.child("AppUsers").child(user.getPhone()).setValue(user.getInfo());
        databaseRef.child("AppUsers").child(user.getPhone()).child("location").setValue(user.getLocation());
    }

    private static void updateFirebaseDB(String oldPhone, AppUser user) {
        // update Firebase DB Online
        DatabaseReference databaseRef;
        databaseRef = FirebaseDatabase.getInstance().getReference();
        // remove children with old phone
        databaseRef.child("AppUsers").child(oldPhone).removeValue();
        databaseRef.child("AppUsers").child(user.getPhone()).child("location").child(oldPhone).removeValue();
