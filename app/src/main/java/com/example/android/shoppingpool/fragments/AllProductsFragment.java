...
//        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ProductsRef = FirebaseDatabase.getInstance().getReferenceFromUrl(strtext);
        productsList = (RecyclerView) view.findViewById(R.id.allProductsRecyclerView);
        productsList.setHasFixedSize(true);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
        linearLayoutManager.setReverseLayout(true);
        linearLayoutManager.setStackFromEnd(true);
        productsList.setLayoutManager(linearLayoutManager);
        progressBar = view.findViewById(R.id.allProductsProgressBar);
    ...
    private void DisplayAllProducts() {
        ...
        firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<AllProducts, ProdutsViewHolder>(options) {
            @Override
            protected void onBindViewHolder(@NonNull ProdutsViewHolder holder, int position, @NonNull AllProducts model) {
    ...
    public static class ProdutsViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setProductVariant(String productVariant)
        {
            ...
            for(i=count+1;i<=7;i++){
                if(i==7)
                {
                    variant7.setVisibility(View.GONE);
                }
                if(i==6)
                {
                    variant6.setVisibility(View.GONE);
                }
                if(i==5)
                {
                    variant5.setVisibility(View.GONE);
                }
                if(i==4)
                {
                    variant4.setVisibility(View.GONE);
                }
                if(i==3)
                {
                    variant3.setVisibility(View.GONE);
                }
                if(i==2)
                {
                    variant2.setVisibility(View.GONE);
                }
                if(i==1)
                {
        ...
        public void setPrice (String discountPercent, String Price, String Offer)
        {
            ...
            {
                if(discountPercent!=null && discountPercent.charAt(discountPercent.length()-1)=='%'){
                    discountPercent = discountPercent.substring(0,discountPercent.length()-1);
                }

                if(discountPercent!=null && Double.parseDouble(discountPercent) == 0.0)
                {
                    offer.setText(Offer);
                    StringTokenizer p=new StringTokenizer(Price,",");
                    String s="";
